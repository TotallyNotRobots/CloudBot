...
# below is the default factoid in every channel you can modify it
# however you like
...
def remember(text, nick, db, chan, notice, event):
    ...
    If <user> in in the message it will be replaced by input arguments when
    command is called.
    """
    try:
    ...
    if data.startswith("+") and old_data:
        ...
        fmt = "Remembering \x02{0}\x02 for \x02{1}\x02. Type {2}{1} to see it."
        notice(fmt.format(data, word, FACTOID_CHAR))
        if old_data:
