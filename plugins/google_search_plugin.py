...
def forecast(text):
    ...
    if response.status_code == 200:
        ...
        for day in j['weather']:
            message.append(f"\x02{day['date']}\x02 - \x02average\x02: {day['avgtempC']}ºC, \x02max\x02: {day['maxtempC']}ºC, \x02min\x02: {day['mintempC']}ºC, \x02sun hours\x02: {day['sunHour']}, \x02precipitation\x02: {round(sum([float(h['precipMM']) for h in day['hourly']]), 3)}mm \x02")
        return message
    else:
...
def astronomy(text):
    ...
    if response.status_code == 200:
        j = response.json()
        nearest = j['nearest_area'][0]
        message = [f"{nearest['country'][0]['value']} - {nearest['region'][0]['value']}, lat: {nearest['latitude']}  long: {nearest['longitude']}"]
        for day in j['weather']:
            ast = day['astronomy'][0]
            MOONS = {
                "First Quarter": "🌓",
            ...
            message.append(f"\x02{day['date']}\x02 - {moon}" + ", ".join([f"\x02{key}\x02: {ast[key]}" for key in ast]))
        return message
    else:
        return "City not found."
