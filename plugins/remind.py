...
def delete_reminder(event, network, remind_time, user):
    query = table.delete() \
...
def delete_all(event, network, user):
    query = table.delete() \
        .where(table.c.network == network.lower()) \
        .where(table.c.added_user == user.lower())
    with event.db_session() as db:
...
def add_reminder(event, network, added_user, added_chan, message, remind_time, added_time):
...
def load_cache(event):
    ...
    for row in rows:
        reminder_cache.append(
            (row["network"], row["remind_time"], row["added_time"], row["added_user"], row["message"]))


@hook.periodic(30, initial_interval=30)
def check_reminders(bot, event):
    ...
    for reminder in reminder_cache:
        ...
        if remind_time <= current_time:
            ...
            delete_reminder(event, network, remind_time, user)
            load_cache(event)


@hook.command('remind', 'reminder', 'in')
def remind(text, nick, chan, conn, notice, event):
    ...
    if text == "clear":
        ...
        delete_all(event, conn.name, nick)
        load_cache(event)
    ...
    add_reminder(event, conn.name, nick, chan, message, remind_time, current_time)
    load_cache(event)
