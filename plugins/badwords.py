...
TOO_MANY_WORDS = (
    ...
    "Please remove a word using .rmbad before adding "
...
def add_bad(text, nick, db):
    """
    <word> <channel> - adds a bad word to the auto kick list must specify a
    channel with each word
    """
    ...
    if len(badcache[channel]) >= 10:
        return TOO_MANY_WORDS.format(channel)
...
def del_bad(text, db):
    """
    <word> <channel> - removes the specified word from the specified channels
    bad word list
    """
    splt = text.lower().split(None, 1)
    word, channel = splt
    ...
    db.execute(
        table.delete().where(
            and_(table.c.word == word, table.c.chan == channel)
        )
    )
    db.commit()
...
def list_bad(text):
    """
    <channel> - Returns a list of bad words specify a channel to see words for
