...
LANGCODE_MSG = (
    "Here is information on what I can translate as well as "
...
def list_langs():
    """
    - List the languages/codes that can be used to translate.
    Translation is powered by Yandex https://translate.yandex.com"""
    api_key = bot.config.get_api_key("yandex_translate")
    if not api_key:
    ...
    return LANGCODE_MSG.format(paste)
...
def trans(text, reply, event):
    ...
    if lang.title() in lang_dict.keys():
        ...
        return (
            "Please specify a valid language, language code, to translate "
            "to. Use .langlist for more information "
