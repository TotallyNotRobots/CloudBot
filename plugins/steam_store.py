...
def format_game(app_id, show_url=True):
    """
    Takes a Steam Store app ID and returns a formatted string with data
    about that app ID.
    ...
    if game["is_free"]:
        ...
        # the steam API sends prices as an int like "9999" for $19.99, we
        # divmod to get the actual price
...
def steam(text, reply):
    ...
    result = soup.find(
        "a", {"class": lambda c: "search_result_row" in c.split()}
    )
