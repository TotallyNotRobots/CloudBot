...
RETRY_MSG = (
    "Qur'an API is having problems, it would be best to " "check back later."
)
...
def smart_truncate(content, length=425, suffix="..."):
    ...
    rsplit = content[:length].rsplit(" ", 1)
    first_message = rsplit[0] + suffix
    other_parts = smart_truncate(rsplit[1] + content[length:])
    return (first_message, *other_parts)


@hook.command("quran", "verse", singlethread=True)
def quran(text, message, reply):
    """
    <verse> - Prints the specified Qur'anic verse(s) and its/their
    translation(s)
    """
