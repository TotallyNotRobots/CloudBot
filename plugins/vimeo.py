...
def vimeo_url(match):
    ...
    data = http.get_json('http://vimeo.com/api/v2/video/%s.json'
                         % match.group(1))
    ...
    info["stats_number_of_likes"] = format(
        info["stats_number_of_likes"], ",d")
    info["stats_number_of_plays"] = format(
        info["stats_number_of_plays"], ",d")
