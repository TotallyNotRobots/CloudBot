...
score_msg = "{} has a total score of {} (+{}/{}) in {}."
score_global_msg = (
    "{} has a total score of {} (+{}/{}) across all channels I know about."
)
...
def points_cmd(text, chan, db):
    ...
    if text.endswith(("-global", " global")):
        ...
        query = select([karma_table.c.score]).where(
            and_(
                karma_table.c.thing == text.lower(), karma_table.c.chan == chan
            )
        )
    ...
    if karma:
        ...
        if thing:
            return score_global_msg.format(thing, score, pos, neg)

        return score_msg.format(text, score, pos, neg, chan)
...
def pointstop(text, chan, db):
    """
    - prints the top 10 things with the highest points in the channel. To see
    the top 10 items in all of the channels the bot sits in use .topten global.
    """
...
def pointsbottom(text, chan, db):
    """
    - prints the top 10 things with the lowest points in the channel.
    To see the bottom 10 items in all of the channels the bot sits in
    use .bottomten global.
    """
