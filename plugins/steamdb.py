...
def get_data(user, currency="us"):
    """
    Takes a steam user ID and returns a dict containing info about the
    games the user owns.
    ...
    try:
        ...
        if cfscrape:
            ...
            fmt = (
                "Could not get user info: {} (You may have been blocked by "
                "CloudFlare, try installing the cfscrape module)"
            )
            raise SteamError(fmt.format(e))
...
def steamcalc(text, reply):
    """
    <username> - Gets value of steam account.
    ...
    fmt = (
        "\x02{name}\x02 has \x02{count:,}\x02 games with a total value of "
        "\x02{value}\x02 (\x02{value_sales}\x02 during sales). "
        "\x02{count_unplayed:,}\x02 (\x02{percent_unplayed}%\x02) have "
        "never been played - {short_url}"
    )
