...
def find_best_code() -> (Question, Answer):
    global results, no_good_code
    if not no_good_code:
        rcopy = deepcopy(results)
        for result in rcopy:
...
def sosearch(query: str, tag: str = None) -> str:
    global results
...
def sosearchhook(text, reply):
    ...
    global results, no_good_code
    no_good_code = False
    if text.split()[0] in TAGS:
        sosearch(" ".join(text.split()[1:]), text.split()[0])
    else:
        sosearch(text)
    if len(results) == 0:
