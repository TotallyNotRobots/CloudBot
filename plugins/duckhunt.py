...
DUCK_HUNT_ENABLED = (
    "Duck hunt is enabled in {}. To disable it run .hunt_opt_out add #channel"
)

DUCK_HUNT_DISABLED = (
    "Duck hunt is disabled in {}. To re-enable it run "
...
def start_hunt(db, chan, message, conn):
    """
    - This command starts a duckhunt in your channel, to stop the hunt
    use .stophunt
    """
...
def set_ducktime(chan, conn):
    ...
    now = int(time())
    status.next_duck_time = random.randint(now + 480, now + 3600)
    # status.flyaway = status.next_duck_time + 600
    status.duck_status = 0
    # let's also reset the number of messages said and the list of
    # masks that have spoken.
...
def no_duck_kick(db, text, chan, conn, notice_doc):
    """
    <enable|disable> - If the bot has OP or half-op in the channel you can
    specify .duckkick enable|disable so that people are kicked for
    shooting or befriending a non-existent goose. Default is off.
    ...
    if text.lower() == "enable":
        ...
        return (
            "users will now be kicked for shooting or befriending non-existent "
            "ducks. The bot needs to have appropriate flags to be able to kick "
...
def generate_duck():
    """
    Try and randomize the duck message so people can't highlight on it/script
    against it.
    """
...
def attack(event, nick, chan, db, conn, attack_type):
    ...
    if attack_type == "shoot":
        ...
        miss = [
            ...
            (
                "The duck said no, maybe bribe it with some pizza? "
                "Ducks love pizza don't they?"
            ),
        ...
        msg = (
            "{} you befriended a duck in {:.3f} seconds! You have made "
            "friends with {} in {}."
        )
        scripter_msg = (
            "You tried friending that duck in {:.3f} seconds, that's "
            "mighty fast. Are you sure you aren't a script? Take a 2 hour "
            "cool down."
        )
        attack_type = "friend"
    ...
    if nick.lower() in scripters:
        if scripters[nick.lower()] > shoot:
            event.notice(COOL_DOWN_MSG.format(scripters[nick.lower()] - shoot))
            return
...
def befriend(nick, chan, db, conn, event):
    """
    - when there is a duck on the loose use this command to befriend it before
    someone else shoots it.
...
def hunt_opt_out(text, chan, db, conn):
    """
    [{add <chan>|remove <chan>|list}] - Running this command without any
    arguments displays the status of the current channel.
    hunt_opt_out add #channel will disable all duck hunt commands
    in the specified channel. hunt_opt_out remove #channel will re-enable
    the game for the specified channel.
    ...
    if not text:
        if is_opt_out(conn.name, chan):
            return DUCK_HUNT_DISABLED.format(chan)

        return DUCK_HUNT_ENABLED.format(chan)
...
def duck_merge(text, conn, db, message):
    """<user1> <user2> - Moves the duck scores from one nick to another nick.
    Accepts two nicks as input the first will have their duck scores removed
    the second will have the first score added. Warning this cannot be undone.
    ...
    if oldnickscore:
        if newnickscore:
            ...
            for row in oldnickscore:
                ...
                duckmerge[row["chan"]]["befriend"] = row["befriend"]
                channelkey["insert"].append(row["chan"])
                # TODO: Call dbupdate() and db_add_entry for the items
                #  in duckmerge
...
def ducks_user(text, nick, chan, conn, db, message):
    """
    <nick> - Prints a users duck stats. If no nick is input it will check
    ...
    if scores:
        ...
        fmt = (
            "\x02{}'s\x02 duck stats: \x02{}\x02 killed and \x02{}\x02 "
            "befriended in {}. Across {}: \x02{}\x02 killed and \x02{}\x02 "
            "befriended. Averaging \x02{}\x02 and \x02{}\x02 per channel."
        )
        message(
            fmt.format(
                name,
...
def duck_stats(chan, conn, db, message):
    ...
    if scores:
        ...
        fmt = (
            "\x02Duck Stats:\x02 {:,} killed and {:,} befriended "
            "in \x02{}\x02. Across {} \x02{:,}\x02 ducks have been killed and "
            "\x02{:,}\x02 befriended. \x02Top Channels:\x02 \x02{}\x02 "
            "with {} and \x02{}\x02 with {}"
        )
        message(
            fmt.format(
                ducks["chankilled"],
