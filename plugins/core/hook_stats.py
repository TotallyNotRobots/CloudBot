"""
Tracks successful and errored launches of all hooks, allowing users
to query the stats
...
def make_hook_counter():
    return defaultdict(default_hook_counter)
...
def get_stats(bot):
    try:
        ...
        bot.memory["hook_stats"] = stats = {
            "global": make_hook_counter(),
            "network": defaultdict(make_hook_counter),
            "channel": defaultdict(lambda: defaultdict(make_hook_counter)),
        }
...
def hookstats(text, bot, notice_doc):
    """
    {global|network <name>|channel <network> <channel>|hook <hook>} -
