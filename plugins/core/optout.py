...
async def optout(text, event, chan, db, conn):
    """
    [chan] <pattern> [allow] - Set the global allow option for hooks matching
    <pattern> in [chan], or the current channel if not specified
...
async def deloptout(text, event, chan, db, conn):
    """
    [chan] <pattern> - Delete global optout hooks matching <pattern> in
    [chan], or the current channel if not specified
    """
    args = text.split()
    if len(args) > 1:
        chan = args.pop(0)
...
async def list_optout(conn, event, async_call):
    """
    [channel] - View the opt out data for <channel> or the current channel
    if not specified. Specify "global" to view all data for this network
...
async def clear(conn, event, db, async_call):
    """
    [channel] - Clears the optout list for a channel. Specify "global" to
