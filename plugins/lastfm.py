...
NO_USERNAME = (
    "No last.fm username specified and no last.fm username is set "
    "in the database."
)
...
def filter_tags(tags, artist, limit=4):
    ...
    # Perform the actual filtering, stop when we reach the desired number of
    # tags
    filtered_tags = []
    for tag in tags:
...
def check_key_and_user(nick, text):
    ...
    :param text: The text passed to the command, possibly a different
        username to use
...
def lastfm(event, db, text, nick):
    """
    [user] [dontsave] - displays the now playing (or last played) track of
    LastFM user [user]
    """
    api_key = bot.config.get_api_key("lastfm")
    if not api_key:
    ...
    if isinstance(tracks, list):
        ...
        if (
            ...
            # if the user is listening to something, the first track (a dict)
            # of the tracks list will contain an item with the "@attr" key.
...
def getuserartistplaycount(event, text, nick):
    """
    [artist] - displays the current user's playcount for [artist]. You must
    have your username saved.
    """
    api_key = bot.config.get_api_key("lastfm")
    if not api_key:
        return "error: no api key set"
...
def lastfmcompare(text, nick):
    """
    <user1> [user2] - displays the now playing (or last played) track of
    LastFM user [user]
    """
    api_key = bot.config.get_api_key("lastfm")
    if not api_key:
    ...
    fmt = (
        "Musical compatibility between \x02{}\x02 and \x02{}\x02: {} "
        "(\x02{}%\x02) {}"
    )
    return fmt.format(
...
def topartists(text, nick):
    ...
    You can set your lastfm username with .l username
    """
...
def topweek(text, nick):
    """
    [username] - Grabs a list of the top artists in the last week for a
    last.fm username. You can set your lastfm username with .l username
    """
...
def topmonth(text, nick):
    """
    [username] - Grabs a list of the top artists in the last month for a
    last.fm username. You can set your lastfm username with .l username
    """
...
def topall(text, nick):
    """
    [username] - Grabs a list of the top artists in the last year for a
    last.fm username. You can set your lastfm username with .l username
    """
