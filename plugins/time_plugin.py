...
VALID_TZ = "Please specify a valid UTC/GMT format Example: UTC-4, UTC+7 GMT7"
BAD_TZ = (
    "Sorry I could not parse the UTC format you entered. Example UTC7 or UTC-4"
)
BEATS_INFO = (
    "1 day = 1000 .beats, 1 hour = 41.666 .beats, 1 min = 0.6944 .beats, "
...
# Change this to a ccTLD code (eg. uk, nz) to make results more targeted
# towards that specific country.
...
def check_status(status, api):
    """
    A little helper function that checks an API error code and
    returns a nice message. Returns None if no errors found
    """
...
def time_command(text, reply):
    ...
    # I'm telling the time module to parse the data as GMT, but whatever,
    # it doesn't matter what the time module thinks the timezone is.
    # I just need dumb time formatting here.
