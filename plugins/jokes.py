...
def load_joke_file(path):
    """
    Loads all the lines from a file, excluding blanks and lines that
...
def load_jokes(bot):
    ...
    Put 'NAME.txt' in `file_list` to make those strings available
    as `joke_lines['NAME']`.
...
def confucious(message):
    ...
    (Note that the spelling is deliberate:
    https://www.urbandictionary.com/define.php?term=Confucious)
    """
...
def boobies(text):
    ...
    if out == text.strip():
        fmt = "Sorry I couldn't turn anything in '{}' into boobs for you."
...
def awesome(text, is_nick_valid):
    ...
    fmt = (
        "{}: I am blown away by your recent awesome action(s). "
        "Please read \x02{}\x02"
    )
    return fmt.format(target, link)
