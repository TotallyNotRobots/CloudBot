...
def forecast(text):
    ...
    if response.status_code == 200:
        ...
        for day in j['weather']:
            message.append(f"\x02{day['date']}\x02 - \x02average\x02: {day['avgtempC']}ºC, \x02max\x02: {day['maxtempC']}ºC, \x02min\x02: {day['mintempC']}ºC, \x02sun hours\x02: {day['sunHour']}, \x02precipitaion\x02: {sum([float(h['precipMM']) for h in day['hourly']])}mm \x02")
        return message
    else:
