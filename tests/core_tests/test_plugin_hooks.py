...
def test_hook_kwargs(hook, reset_database):
    assert (
...
def test_hook_doc(hook, reset_database):
...
def test_hook_args(hook, reset_database):
    bot = MockBot()
    if hook.type in (
        "irc_raw",
        "perm_check",
...
def test_coroutine_hooks(hook, reset_database):
    if inspect.isgeneratorfunction(hook.function):  # pragma: no cover
        assert asyncio.iscoroutinefunction(hook.function), (
            "Non-coroutine generator function used for a hook. "
            "This is most liekly due to incorrect ordering of the "
