...
def test_get_plugin(mock_manager):
    ...
    obj = plugin.Plugin(str(file_path), file_name, "test", MockModule(),)
...
async def _test_weird_obj(patch_import_module, mock_manager, weird_obj):
...
@pytest.mark.asyncio
async def test_plugin_with_objs_none_attr(mock_manager, patch_import_module):
    await _test_weird_obj(
        patch_import_module, mock_manager, WeirdObject(lambda *args: None)
    )


@pytest.mark.asyncio
async def test_plugin_with_objs_mock_attr(mock_manager, patch_import_module):
    await _test_weird_obj(
        patch_import_module,
        mock_manager,
...
@pytest.mark.asyncio
async def test_plugin_with_objs_dict_attr(mock_manager, patch_import_module):
    await _test_weird_obj(
...
@pytest.mark.asyncio
async def test_plugin_with_objs_full_dict_attr(
...
@pytest.mark.asyncio
async def test_plugin_load_disabled(
    mock_manager, patch_import_module, patch_import_reload
) -> None:
    ...
    assert await (mock_manager.load_plugin("plugins/test.py")) is None
