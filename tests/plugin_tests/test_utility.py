...
@pytest.mark.parametrize('text,replace,out', [
    ('foo', {'f': 'br'}, 'broo'),
])
...
@pytest.mark.parametrize('text,output', [
    (out, text) for text, out in ESCAPE_DATA
])
...
@pytest.mark.parametrize('text,text_length', [
    ('foo', 3),
    ('foo bar', 7),
    ('Foo', 3),
    ('fOO', 3),
...
@pytest.mark.parametrize('text,out', [
    (
            ...
            ('md5: e23f57ae0441216d5c818ba18c1fc98b, '
             ...
             'sha256: 6afee6de002d236bd35ea0918848b8f85342acd887'
             '499b5e8567598af1e337b4'),
    ),
])
