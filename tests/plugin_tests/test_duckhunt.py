...
def test_display_scores(mock_db):
    ...
    global_res = duckhunt.friends("global", event, chan, conn, session)
    assert global_res == global_friends

    global_kills_res = duckhunt.killers("global", event, chan, conn, session)
    assert global_kills_res == global_kills

    friends_res = duckhunt.friends("average", event, chan, conn, session)
    assert friends_res == average_friends

    kills_average_msg = duckhunt.killers("average", event, chan, conn, session)
    assert kills_average_msg == average_kills
