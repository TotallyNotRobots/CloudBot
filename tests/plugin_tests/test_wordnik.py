...
class WordTestBase:
    ...
    def build_url(cls, word, op=None, paramstring=None):
        ...
        limit = cls.get_result_limit()
        if limit:
            param_trail = "limit={}&api_key=APIKEY".format(limit)
        else:
            ...
            json=[
                {
                    ...
                    "text": (
                        ...
                        "symbolizes and communicates a meaning and may consist "
...
class TestPronounce(WordTestBase):
    ...
    def test_search_audio_error(self, mock_requests, mock_api_keys):
        ...
        event.reply.assert_called_with(expected)
...
class TestSynonym(WordTestBase):
    ...
    @classmethod
    def get_not_found_msg(cls, word):
        fmt = "Sorry, I couldn't find any synonyms for \x02{}\x02."
...
class TestAntonym(WordTestBase):
    ...
    def get_paramstring(cls):
        return (
            "relationshipTypes=antonym&limitPerRelationshipType=5"
            "&useCanonical=false"
        )

    @classmethod
    def get_not_found_msg(cls, word):
        fmt = "Sorry, I couldn't find any antonyms for \x02{}\x02."
        return fmt.format(word)

    def test_no_valid(self, mock_requests, mock_api_keys):
        mock_requests.add(
            "GET",
            self.build_url("clear"),
            match_querystring=False,
            json=([{"relationshipType": "antonym",}] * 60),
        )

        with pytest.raises(wordnik.NoValidResults):
            self.call("clear")

    def test_missing_fields(self, mock_requests, mock_api_keys):
        mock_requests.add(
            "GET",
            self.build_url("clear"),
            match_querystring=False,
            json=([{"relationshipType": "antonym",}] * 60)
            + [{"relationshipType": "antonym", "words": ["foo"]}],
        )

        expected = "\x02clear\x02: foo"
    ...
    def test_search(self, mock_requests, mock_api_keys):
        ...
        mock_requests.add(
            "GET", self.build_url("clear"), match_querystring=True, json=body,
        )
...
class WordsTestBase(WordTestBase):
    ...
    def build_url(cls, word=None, op=None, paramstring=None):
        ...
        params = (paramstring or cls.get_paramstring()), "api_key=APIKEY"
        return url + "?" + "&".join(filter(None, params))
...
class TestWOTD(WordsTestBase):
    ...
    def test_today(self, mock_requests, mock_api_keys):
        definitions = [
            {
                "source": "wiktionary",
                "text": (
                    "(in science fiction) a psychic ability to control "
                    "electronic machinery and/or read electronic signals, "
                    "especially hardware."
                ),
                "note": None,
                "partOfSpeech": "noun",
        ...
        example_url = (
            "http://www.superheronation.com/2009/08/25/foxs-" "review-forum/"
        )
        examples = [
            {
                "url": example_url,
                ...
                "text": (
                    ...
                    "probability manipulation, super intelligence, and "
                    "technopathy."
                ),
                "id": 1101722947,
            }
        ...
        expected = (
            "The word for \x02today\x02 is \x02technopathy\x02: "
            "\x0305(noun)\x0305 \x0310The word 'technopathy' comes from "
            "Greek roots meaning 'skill' and 'suffering'.\x0310 "
            "\x02Definition:\x02 \x0303(in science fiction) a psychic ability "
            "to control electronic machinery and/or read electronic signals, "
            "especially hardware.\x0303"
        )
    ...
    def test_date(self, mock_requests, mock_api_keys):
        definitions = [
            ...
            {
                "text": (
                    "Wine, more especially red wine diluted with water, "
                    "sweetened, and flavored with nutmeg, used as a cold "
                    "drink. Varieties of it are named from the wine employed: "
                    "as, port-wine sangaree."
                ),
                "partOfSpeech": "noun",
                "source": "century",
                "note": None,
        ...
        examples = [
            {
                "url": (
                    ...
                    "7767857a6a9b9e4f378ab2fdf73d93ca6fffc02a3e50cd92be465"
                    "d0e2276df30"
                ),
                "text": (
            ...
            },
            {
                "url": (
                    "http://api.wordnik.com/v4/mid/"
                    "7e003ed62c2faddb52212251bd929b1d67b24a69fda3a0c2b2efece"
                    "1b6008e90"
                ),
                "text": (
                    ...
                    "ice, -- ice this time, -- and sugar, and limes, and "
                    "slices of pineapple, Madam, -- the which he had "
                    "concocted during our slumber."
                ),
                "title": "The Atlantic Monthly, Volume 15, No. 87, "
                "January, 1865",
                "id": 1087980293,
            },
            {
                "url": (
                    "http://api.wordnik.com/v4/mid/"
                    "b051be319d7935b65edb84e6b1fa74dc1a978ed6787e82d19b7d"
                    "fca343c8c6dc"
                ),
                "text": (
                    "In anticipation of the hot weather, I had laid in a large "
                    "stock of raspberry vinegar, which, properly managed, "
                    "helps to make a pleasant drink; and there was a great "
                    "demand for sangaree, claret, and cider cups, the cups "
                    "being battered pewter pots."
                ),
            ...
            },
            {
                "url": (
                    "http://api.wordnik.com/v4/mid/"
                    "baf84d8de4707ffdfdbc58cc8deb717fe110573f734f4ebdb5a3b2e2"
                    "bccb6777"
                ),
                "text": (
                    "A sangaree or any other delicacy, taken while resting "
                    "after a walk which taxed the weakened energies to the "
                    "utmost, or a meal served outside the fevered air of the "
                    "wards, did more to build up the strength than any amount "
                    "of medicine could have done."
                ),
                "title": (
                    "Memories A Record of Personal Experience and Adventure "
        ...
        mock_requests.add(
            ...
            json={
                ...
                "contentProvider": {"name": "wordnik", "id": 711},
                "note": "The word 'sangaree' is related to the Spanish word "
                "'sangria'.",
                "htmlExtra": None,
                "pdd": "2018-11-21",
                "definitions": definitions,
        ...
        expected = (
            "The word for \x022018-11-21\x02 is \x02sangaree\x02: "
            "\x0305(verb)\x0305 \x0310The word 'sangaree' is related to the "
            "Spanish word 'sangria'.\x0310 \x02Definition:\x02 \x0303To mix "
            "with water and sweeten; make sangaree of: as, to sangaree "
            "port-wine.\x0303"
        )
...
def test_no_min(mock_requests, mock_api_keys):
    mock_requests.add(
        "GET",
        "http://api.wordnik.com/v4/word.json/foobar/definitions?limit=5"
        "&api_key=APIKEY",
        match_querystring=False,
        json=[{"text": "foobar",}],
    )
    def_req = wordnik.DefinitionsLookupRequest("foobar")
    res = list(def_req.get_filtered_results(5))

    assert res == [{"text": "foobar"}]
