...
class PermissionManager(object):
    ...
    def __init__(self, conn):
        ...
        """
        logger.info(
            "[%s|permissions] Created permission manager for %s.", conn.name, conn.name
        )
    ...
    def reload(self):
        ...
        self.perm_users = {}
        logger.info(
            "[%s|permissions] Reloading permissions for %s.", self.name, self.name
        )
        ...
        logger.debug(
            "[%s|permissions] Group permissions: %s", self.name, self.group_perms
        )
        logger.debug("[%s|permissions] Group users: %s", self.name, self.group_users)
        logger.debug(
